import numpy as np
from ..xp_compat import cp, HAS_GPU, resolve_device
from .kernel_manager import KernelManager
from .blank_hits_cpu import blank_hits_kernel_cpu
...
class BlankHitsMan(KernelManager):
    ...
    def init(self, N_time: int, N_beam: int, N_chan: int, N_blank: int, device: str='gpu'):
        ...
        if reinit:
            logger.debug(f"BlankHitsMan: Reinitializing (device={device})")
            self.N_chan    = N_chan
            self.N_beam    = N_beam
            self.N_time    = N_time
    ...
    def execute(self, d_gpu, cidxs_gpu, dd_shift_gpu, 
                N_pad_lower, N_pad_upper):
        """ Execute kernel on dedoppler data array 
        
        Despite the legacy '_gpu' suffix in argument names (kept for
        backward compatibility), these can be plain numpy arrays when
        self.device == 'cpu'.
        """
        if self.device == 'gpu':
            ...
            blank_hits_kernel_cpu(
                d_gpu,
                np.asarray(cidxs_gpu).astype(np.int32),
                np.asarray(dd_shift_gpu).astype(np.int32),
                np.asarray(N_pad_lower).astype(np.int32),
                np.asarray(N_pad_upper).astype(np.int32),
                self.N_chan, self.N_time,
            )
