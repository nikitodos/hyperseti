...
from ..xp_compat import cp, get_xp, asnumpy, resolve_device, HAS_GPU
from .kernel_manager import KernelManager
from .peak_finder_cpu import find_max_1d_cpu, find_max_reduce_cpu
...
def find_max_1D(d_gpu, maxval_gpu, maxidx_gpu):
    ...
    GPU-only: called by PeakFinderMan.execute() only when self.device ==
    'gpu'. The CPU path uses find_max_1d_cpu (kernels/peak_finder_cpu.py)
    instead, which is not a thin wrapper around a RawKernel launch and so
...
def find_max_reduce(maxval_gpu, maxidx_gpu, maxval_k_gpu,
                    maxidx_f_gpu, maxidx_t_gpu, K: int):
    ...
    GPU-only, see find_max_1D docstring.
...
class PeakFinderMan(KernelManager):
    ...
    def init(self, N_chan: int, N_time: int, K: int, device: str='gpu'):
        ...
        # Allocate workspace (GPU: cupy, CPU: numpy)
        self.workspace['maxval_gpu'] = xp.asarray(maxval)
        self.workspace['maxidx_gpu'] = xp.asarray(maxidx)
        self.workspace['maxval_k_gpu'] = xp.asarray(maxval_k)
        self.workspace['maxidx_f_gpu'] = xp.asarray(maxidx_f)
        self.workspace['maxidx_t_gpu'] = xp.asarray(maxidx_t)

    def execute(self, d_gpu):
        """ Execute peak finder """
        try:
        ...
        if self.device == 'gpu':
            ...
            maxval, maxidx = find_max_1d_cpu(d_gpu)
            maxval_k, maxidx_f, maxidx_t = find_max_reduce_cpu(maxval, maxidx, self.K)
            ws['maxval_gpu'][:] = maxval
            ws['maxidx_gpu'][:] = maxidx
            ws['maxval_k_gpu'][:] = maxval_k
            ws['maxidx_f_gpu'][:] = maxidx_f
            ws['maxidx_t_gpu'][:] = maxidx_t

        return ws['maxval_k_gpu'], ws['maxidx_f_gpu'], ws['maxidx_t_gpu']

    def find_peaks(self, d_gpu, return_space: str='cpu'):
        """ Find peaks in data

        Args:
            d_gpu (np.ndarray or cp.ndarray): 2D data array to search
        """
        ws = self.workspace
        self.execute(d_gpu)
        if return_space != 'cpu':
            ...
            return asnumpy(ws['maxval_k_gpu']), asnumpy(ws['maxidx_f_gpu']), asnumpy(ws['maxidx_t_gpu'])

    def hitsearch(self, d_arr, threshold: float, min_spacing: float, beam_id: int=0, return_space: str='cpu'):
        """ Find peaks in data above threshold

        Also applies a third-stage filter to ensure hits have a minimum spacing

        Args:
            d_arr (np.ndarray or cp.ndarray): (time, beam, freq) data array to search
        """
        xp = get_xp(d_arr)
        if d_arr.shape[1] > 1:
        ...
        hits  = asnumpy(self.workspace['maxval_k_gpu'][mask])
        idx_f = asnumpy(self.workspace['maxidx_f_gpu'][mask])
        idx_t = asnumpy(self.workspace['maxidx_t_gpu'][mask])
...
def peak_find(dedopp_data, threshold: float, min_spacing: float, beam_id: int=0, mm: PeakFinderMan=None):
        """ Find peaks in data above threshold

        Also applies a third-stage filter to ensure hits have a minimum spacing

        Args:
            dedopp_data (np.ndarray or cp.ndarray): (dedopp, beam, freq) data array to search
        """
